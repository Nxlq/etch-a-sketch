...
window.addEventListener('load', function(){
    ...
    // custom trail style settings
    let customGridTrailColor = colorPicker.value;

    // Grid size settings (which correlates with etch trail size)
    ...
    // set default settings and allow for dynamic style selections
    let currentGridTrailColor = classicGridTrailColor;
    let currentGridColor = classicGridColor;
    let currentGridSize = smallTrailSize;


    // allows for functionality to only draw when mouse is clicked and dragged, not just dragged
    let mouseDown = false;
